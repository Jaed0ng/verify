...
// For reg_t:
#include "decode.h"
// For std::unordered_map C++标准库提供的无序哈希表容器  引入无序哈希表容器（std::unordered_map）
// unordered_map是一个键值对（key-value）容器，核心特点：哈希表无序存储（对比std::map为红黑树有序），内存开销较大，但是查找速度快，复杂度为1（std::map为logN）
#include <unordered_map>
// For std::shared_ptr 引入共享智能指针 定义了内存管理相关的工具，其中最常用的就是各种智能指针（shared_ptr、unique_ptr、weak_ptr 等）。
//共享智能指针，核心作用：自动管理动态分配的内存，避免内存泄漏，同时支持多个指针共享同一个对象。
//代码中定义 mvip_csr_t_p 是 mvip_csr_t 的共享智能指针类型，原因是：
//mvip_csr_t 是 RISC-V 的 MVIP 寄存器类，属于处理器核心（processor_t）的关键组件；
//处理器的多个模块（如中断控制器、虚拟化模块）可能需要同时访问 MVIP 寄存器实例；
//使用 std::shared_ptr 可以：
//    自动释放内存：当没有任何模块引用这个 MVIP 寄存器实例时，shared_ptr 会自动调用析构函数，释放内存（避免手动 delete 导致的内存泄漏）；
//    安全共享：多个模块可以通过 shared_ptr 访问同一个 MVIP 实例，无需担心 “重复释放” 或 “野指针” 问题。
//例如：普通指针 mvip_csr_t* p = new mvip_csr_t(proc, addr, init); 
// 问题1：需要手动 delete，忘记则内存泄漏
// 问题2：多个模块共享 p 时，可能重复 delete 导致崩溃
//delete p; 
//
//// 智能指针（无需手动 delete）
//std::shared_ptr<mvip_csr_t> p = std::make_shared<mvip_csr_t>(proc, addr, init);
// 当 p 超出作用域/没有任何引用时，自动释放内存
// 多个模块复制 p 时，内部引用计数+1，不会重复释放
// CSR 寄存器（如 MVIP）是处理器核心的 “全局资源”，中断模块、虚拟化模块、调试模块都可能访问；
// 智能指针符合 Spike 的 “面向对象 + 资源安全” 设计理念，避免手动管理内存导致的 bug（模拟器运行时内存泄漏会导致性能下降甚至崩溃）。
// 
#include <memory>
// For std::optional
#include <optional>
