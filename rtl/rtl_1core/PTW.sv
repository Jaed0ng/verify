...
module PTW(
  ...
  wire             _GEN_26 = (&io_req_bits_req_info_s2xlate) & check_gpa_high_fail_1;
  wire             _GEN_27 = io_hptw_req_ready & io_hptw_req_valid_0;
  wire             _GEN_28 = _GEN_27 & ~s_hptw_req;
  wire             _GEN_29 =
  ...
  wire             _GEN_30 =
    ...
    & (_GEN_6
         ...
         : _GEN_22);
  wire             _GEN_31 = _GEN_27 & ~s_last_hptw_req;
  wire             _GEN_32 = sent_to_pmp & ~mem_addr_update;
  wire             _GEN_33 = accessFault & ~io_hptw_req_valid_0 & ~idle;
  wire             _GEN_34 = guestFault & ~idle;
  wire             _GEN_35 = _GEN_34 | _GEN_33;
  wire             _GEN_36 = io_mem_resp_valid & ~w_mem_resp;
  wire             _GEN_37 = ~_GEN_36 & (_GEN_35 | s_llptw_req);
  wire             _GEN_38 =
    _GEN_36 | _GEN_34 | _GEN_33 | _GEN_13 | _GEN_10 & _GEN_29
    | (_GEN_6
         ? (_check_gpa_high_fail_T
              ? check_gpa_high_fail | mem_addr_update
              : _GEN_26 | mem_addr_update)
         : mem_addr_update);
  wire             _GEN_39 = mem_addr_update & _GEN_15;
  wire             _GEN_40 = io_llptw_ready & io_llptw_valid_0;
  wire             _GEN_41 = ~mem_addr_update | _GEN_15;
  wire             _GEN_42 = guestFault | accessFault | pageFault | ppn_af;
  wire             _GEN_43 = io_resp_valid_0 & _GEN_23;
  wire             _GEN_44 = ~_GEN_16 & _GEN_43;
  wire             _GEN_45 = _GEN_15 | io_llptw_valid_0 | _GEN_16;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      s_pmp_check <=
        flush | _GEN_34 | _GEN_33 | _GEN_32 | ~(_GEN_10 & ~_GEN_29)
        & (~_GEN_6 | _check_gpa_high_fail_T | (&io_req_bits_req_info_s2xlate))
        & s_pmp_check;
      s_mem_req <=
        flush | (~_GEN_39 | s2xlate) & (perfEvents_4_2 | _GEN_35 | ~_GEN_32 & s_mem_req);
      s_llptw_req <=
        flush
        | (mem_addr_update
             ? _GEN_15 | (io_llptw_valid_0 ? _GEN_40 | _GEN_37 : _GEN_44 | _GEN_37)
             : _GEN_37);
      w_mem_resp <= flush | _GEN_36 | ~perfEvents_4_2 & (_GEN_35 | w_mem_resp);
      s_hptw_req <=
        flush | ~(mem_addr_update & _GEN_15 & s2xlate)
        & (_GEN_34 | _GEN_33 | _GEN_28
           | (~_GEN_6 | _check_gpa_high_fail_T | ~(&io_req_bits_req_info_s2xlate)
              | check_gpa_high_fail_1) & s_hptw_req);
      w_hptw_resp <= flush | _GEN_34 | _GEN_33 | _GEN_10 | ~_GEN_28 & w_hptw_resp;
      s_last_hptw_req <=
        flush | (~mem_addr_update | _GEN_15 | io_llptw_valid_0 | ~(_GEN_16 & ~_GEN_42))
        & (_GEN_34 | _GEN_33 | _GEN_31
           | (~(_GEN_6 & _check_gpa_high_fail_T) | check_gpa_high_fail) & ~_GEN_5
           & s_last_hptw_req);
      w_last_hptw_resp <=
        flush | _GEN_34 | _GEN_33 | _GEN_14 | ~_GEN_31 & w_last_hptw_resp;
      mem_addr_update <=
        ~flush
        & (mem_addr_update
             ? ~_GEN_15
               & (io_llptw_valid_0
                    ? ~_GEN_40 & _GEN_38
                    : _GEN_16 ? _GEN_42 & _GEN_38 : ~_GEN_43 & _GEN_38)
             : _GEN_38);
      idle <=
        flush
        | (_GEN_41 ? _GEN_25 : io_llptw_valid_0 ? _GEN_40 | _GEN_25 : _GEN_44 | _GEN_25);
      if (_GEN_13)
      ...
      if (_GEN_41)
        need_last_s2xlate <= ~_GEN_35 & _GEN_30;
      else if (io_llptw_valid_0)
        need_last_s2xlate <= ~(_GEN_40 | _GEN_35) & _GEN_30;
      else
        need_last_s2xlate <= ~(_GEN_16 | _GEN_35) & _GEN_30;
      first_gvpn_check_fail <=
        ~flush & (~mem_addr_update | _GEN_45 | ~_GEN_43)
        & (_GEN_6
             ? (_check_gpa_high_fail_T
                  ? check_gpa_high_fail | first_gvpn_check_fail
                  : _GEN_26 | first_gvpn_check_fail)
             : first_gvpn_check_fail);
      pte_valid <= _GEN_36 | ~_GEN_6 & pte_valid;
      accessFault <=
        ~flush
      ...
      update_full_gvpn_mem_resp <=
        ~update_full_gvpn_mem_resp & (_GEN_36 | update_full_gvpn_mem_resp);
